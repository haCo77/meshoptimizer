...
static bool areMaterialsEqual(const cgltf_material& lhs, const cgltf_material& rhs, const Settings& settings, const std::string& extras)
{
...
void mergeMeshMaterials(cgltf_data* data, std::vector<Mesh>& meshes, const Settings& settings, const std::string& extras)
{
	...
	for (size_t i = 0; i < data->materials_count; ++i)
	{
		...
		for (size_t j = 0; j < i; ++j)
		{
			...
			if (areMaterialsEqual(data->materials[i], data->materials[j], settings, extras))
			{
